...
{
#ifdef USE_USER_UART
  if (__HAL_UART_GET_FLAG(&HANDLE_USER_UART, UART_FLAG_RXNE) != HAL_OK)
  {
    *key = ((uint8_t)HANDLE_USER_UART.Instance->DR & (uint8_t)0xFF);
    return 1;
  }
#else
...
{
#ifdef USE_USER_UART
  HAL_UART_Transmit(&HANDLE_USER_UART, &c, 1, 500);
  //delay_us(100);
#else
